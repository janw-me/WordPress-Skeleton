...
class MY_Vcard_Widget extends WP_Widget {
	function MY_Vcard_Widget() {
		$widget_ops = array('classname' => 'widget_theme1958_vcard', 'description' => __('Use this widget to add a vCard', 'theme1958'));
		$this->WP_Widget('widget_theme1958_vcard', __('My - vCard', 'theme1958'), $widget_ops);
		$this->alt_option_name = 'widget_theme1958_vcard';
	...
	function widget($args, $instance) {
		$cache = wp_cache_get('widget_theme1958_vcard', 'widget');
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('vCard', 'theme1958') : $instance['title'], $instance, $this->id_base);
		...
		<p class="vcard">
			<?php if ($instance['tel'] != '') { ?><span class="tel"><span class="value"><?php echo $instance['tel']; ?></span></span><br><?php } ?>
			<span class="adr">
				<?php if ($instance['street_address'] != '') { ?><span class="street-address"><?php echo $instance['street_address']; ?></span>, <?php } ?>
				<?php if ($instance['locality'] != '') { ?><span class="locality"><?php echo $instance['locality']; ?></span>, <?php } ?>
				<?php if ($instance['region'] != '') { ?><span class="region"><?php echo $instance['region']; ?> </span><?php } ?>
				<?php if ($instance['postal_code'] != '') { ?><span class="postal-code"><?php echo $instance['postal_code']; ?></span><?php } ?>
			</span>
			
			<?php if ($instance['email'] != '') { ?><br><a class="email" href="mailto:<?php echo $instance['email']; ?>"><?php echo $instance['email']; ?></a><?php } ?>
		</p>

		<?php echo $after_widget;

		$cache[$args['widget_id']] = ob_get_flush();
		wp_cache_set('widget_theme1958_vcard', $cache, 'widget');
	}
	...
	function update($new_instance, $old_instance) {
		...
		if (isset($alloptions['widget_theme1958_vcard'])) {
			delete_option('widget_theme1958_vcard');
		}
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_theme1958_vcard', 'widget');
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title (optional):', 'theme1958'); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>">
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('street_address')); ?>"><?php _e('Street Address:', 'theme1958'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('locality')); ?>"><?php _e('City/Locality:', 'theme1958'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('region')); ?>"><?php _e('State/Region:', 'theme1958'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('postal_code')); ?>"><?php _e('Zipcode/Postal Code:', 'theme1958'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('tel')); ?>"><?php _e('Telephone:', 'theme1958'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('email')); ?>"><?php _e('Email:', 'theme1958'); ?></label>
			<input class="widefat" id="<?php echo esc_attr($this->get_field_id('email')); ?>" name="<?php echo esc_attr($this->get_field_name('email')); ?>" type="text" value="<?php echo esc_attr($email); ?>">
		</p>
	<?php }
}

function theme1958_widget_init() {
	register_widget('MY_Vcard_Widget');
}
add_action('widgets_init', 'theme1958_widget_init');