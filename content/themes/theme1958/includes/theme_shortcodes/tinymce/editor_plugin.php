...
$path_to_wp = explode( 'content', $absolute_path );
$wp_url = $path_to_wp[0];

//Access WordPress
require_once( $wp_url.'/wp/wp-load.php' );
				...
				d.addCommand( "myThemeOpenDialog",function(a,c){
					...
					jQuery.get(e+"/dialog.php",function(b){
						...
						switch ( myThemeSelectedShortcodeType ) {
							...
							// dropcap
							case 'dropcap':
								var a = '[dropcap character="Your character goes here" background="#f9b34e"]Your content goes here.[/dropcap]';
								tinyMCE.activeEditor.execCommand("mceInsertContent", false, a);
								break;
							// frame
							case 'frame':
							...
							// timeline
							case 'timeline':
								var a = '[timeline] [event][date] Date Here [/date] Event Here [/event] [/timeline]';
								tinyMCE.activeEditor.execCommand("mceInsertContent", false, a);
								break;
							// indent_left
							case 'indent_left':
								var a = '[indent_left]'+selectedText+'[/indent_left]';
								tinyMCE.activeEditor.execCommand("mceInsertContent", false, a);
								break;
							// Horizontal Ruel
							case 'hr':
							...
							case 'blockquote':
								...
								break;
							case 'box':
								var a = '[box] [img] Your image here... [/img] [text] Your text here... [/text] [/box]';
								tinyMCE.activeEditor.execCommand("mceInsertContent", false, a);
								break;
							// clear
							case 'clear':
		...
		if(d=="shortcodes_button"){
			...
			var a=this;d.onRenderMenu.add(function(c,b){
				...
				c=b.addMenu({title:"HTML"});
					...
					a.addWithDialog(c,"Timeline","timeline");
					a.addWithDialog(c,"Indent Left","indent_left");
